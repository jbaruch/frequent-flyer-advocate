...
- Every expiry decision reads an injectable reference date instead of the wall clock, so no test depends on being ahead of it. Resolves #40.
  - Three `check` tests were green only because their fixtures pinned dates in 2027. `check` filters expired credits, so those dates were load-bearing — swapping them for past ones turned the tests red, which is what the ban on future dates in `testing-standards` Determinism is about: they pass today and start failing on their own once the clock reaches them, with no code change.
  - `reference_date()` reads `CREDITS_TRACKER_TODAY=YYYY-MM-DD`, falling back to the wall clock when unset. All eight `datetime.now()` call sites route through it. The suite freezes it at `2026-03-01`, and every date literal in every suite is now behind the real clock — verified by an audit, not by inspection.
  - A malformed override exits 2 at startup rather than being ignored. Validating lazily was not enough: `cmd_list` returns early on an empty store and never reads a date, so a bad value passed silently on exactly the paths that do not check — and a suite that meant to freeze time would have run against the real clock and passed for the wrong reason.
  - Absence and emptiness are distinguished. `if not raw` treated an explicitly empty `CREDITS_TRACKER_TODAY=` as unset and fell through to the wall clock — but an empty value is a misconfiguration, a shell variable that failed to expand, and reading it as a decision to use real time is the silent fallback this guard exists to refuse. Only `None` is absence now.
  - The validation sits after `parse_args()` and inside the boundary handler. Placed before both, it exited with an empty stdout — breaking the one-object `--json` contract on the very path a caller most needs it — and made `--help` require a valid override, which needs no date at all. `reference_date()` raises now and the boundary emits `invalid_reference_date`.
  - The fallback path is tested against a stubbed clock, not the real one. Comparing a stamped record to `date.today()` at assertion time is itself the wall-clock dependency Determinism bans, and it races the subprocess across midnight. The test swaps the module's `datetime` for a fixed one and asserts that unset reaches it while the override still wins.
  - The seam is load-bearing, not decorative: unfreezing it turns exactly the three tests #40 named red. Its own contract has tests too — the same record reads unexpired at one frozen date and expired at another, a malformed value is fatal, and unset still stamps records with the real date.
- The v4 comment claimed archived used-state fields "ride along verbatim rather than being dropped" while `strip_used_state()` removes them and `migrate` reports them as `dropped_used_state`. Left behind as an advisory when #44 merged; a comment that contradicts the function beneath it is the kind that misleads the next edit.
...
- The `COMP` type is renamed `COMPANION`. Resolves #24.
  ...
  - `SKILL.md` points at `--help` and the script's structured errors instead of naming types. Guidance that lists the accepted set is a copy that drifts, and `script-as-black-box` keeps the vocabulary in the script. The durable version of "do not guess a type from an abbreviation" is the script refusing the guess: Step 11 now documents the four `add` failure payloads and says to read the replacement out of `renamed_to` rather than infer it. A test fails if the skill spells a type token again.
