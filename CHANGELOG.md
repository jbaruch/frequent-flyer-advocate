...
- `credits-tracker.py` stamps a `schema_version` on every record, which `stateful-artifacts` Required Attributes demands and no record carried. Closes #26.
  ...
  - Step 3 gates the chain on `skipped_newer` and `unreadable`. Records the script cannot consume are omitted from every later command, so falling through would have let `list`, `expiring`, and `check` return a subset that reads as the whole store, and `add` write against an inventory it cannot fully see. Either count above zero stops at Step 9 instead. `unreadable` — a version line that does not parse — had no branch at all before; it is now counted, reported, and never guessed at.
  ...
  - `parse_credits()` consumes only records at `SCHEMA_VERSION` exactly, omitting anything off-version in either direction with a per-record stderr warning that names the recovery. Migration Policy has a reader treat an off-version record as read-only "no usable prior state", and every caller other than the owner skill is a non-owner reader — an older record means the owner has not upgraded it yet, and reading it anyway is the non-owner migration the policy forbids. An unversioned record still reads as version 1, which is documented compatibility rather than an off-version record. `next_id()` scans raw `### #` headings rather than that filtered view, so an id is never allocated over a record the script cannot see.
