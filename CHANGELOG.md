...
- The new regression tests were first appended below the module's `if __name__ == "__main__": sys.exit(main())` guard, where `sys.exit` fires before their definitions are ever evaluated. Run under pytest they passed, because an import never reaches the guard; run the way the gate runs the file, the suite reported 95/95 with none of them present. The guard now sits at the end of the module, and both runners report 100.
