...
**v3 → v4** applies the v2 relocation to the archive. v2 scoped its scan to Active, so a miles or points grant that had already been marked used was never classified by its `Value` — it reached the v3 rename still typed `COMP` and came out `COMPANION`, the one label v3's own note says no `COMP` row deserved. v4 re-reads the archive on `Value`, the same authority v2 used, so the outcome does not depend on the label v3 left. A grant is in the loyalty account from the moment it is made, so the used state those rows carry records an event that cannot happen; the fields ride along verbatim rather than being dropped, since somebody wrote them and they are the only record of what was believed at the time. Reported separately from v2's moves as `archived_deposits_relocated`.
